...
void search(State* Obj, const int amount, string str) {
	...
	for (int i = 0; i < amount; i++) {
		if (str == Obj[i].stateName || str == Obj[i].capital || str == Obj[i].population || str == Obj[i].area) {
			cout << left << setw(20) << "№  " << setw(20) << "Государство" << setw(20) << "Столица"
				<< setw(20) << "Население" << setw(20) << "Площадь" << endl;
			cout << "__________________________________________________" << endl;

			cout << left << setw(20) << i + 1 << setw(20) << Obj[i].stateName << setw(20) << Obj[i].capital
				<< setw(20) << Obj[i].area << setw(20) << Obj[i].population << endl;
		}
		else k += 1;
	}
...
void showData(const State* Obj, const int amount) {
	...
	for (int i = 0; i < amount; i++) {
		cout << left << setw(20) << i + 1 << setw(20) << Obj[i].stateName << setw(20) << Obj[i].capital
			<< setw(20) << Obj[i].population << setw(20) << Obj[i].area << endl;
	}
}