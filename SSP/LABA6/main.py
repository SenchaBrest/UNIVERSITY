from PyQt5.QtWidgets import (
    ...
    QMessageBox,
    QFileDialog)
from sql import ClientBookDatabase
import os


class SimpleClientBook(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Simple Client Book")
        ...
        self.textEdit_info = QTextEdit()
        self.gridLayout.addWidget(self.textEdit_info, 1, 1, 6, 2)
        self.textEdit_info.textChanged.connect(self.text_changed)
        ...
        self.current_info = self.textEdit_info.toPlainText()
    ...
    def text_changed(self):
        if self.lineEdit_name.isEnabled():
            name, info = self.get_current_name_and_info()
            if name.strip() != '' and info.strip() != '' and self.textEdit_info.isEnabled() \
                    or name.strip() != '' and not self.textEdit_info.isEnabled():
                if self.current_index is not None:
                    _name, _info = self.get_name_and_info_by_index()
                    if name != _name or info != _info:
    ...
    def display_entry(self):
        if self.current_index is not None:
            ...
            self.textEdit_info.setPlainText(_info)
        else:
    ...
    def make_new_file(self):
        ...
        if self.file_name:
            try:
                ...
                pass
            self.setWindowTitle(f"Simple CLient Book: {self.file_name}")
            self.database = ClientBookDatabase(self.file_name)
    ...
    def add_entry(self):
        ...
        def confirm():
            name, info = self.get_current_name_and_info()
            if not self.database.add_contact(name, info):
    ...
    def edit_entry(self):
        ...
        old_name, _ = self.get_current_name_and_info()

        def confirm():
            new_name, new_info = self.get_current_name_and_info()
            if not self.database.edit_contact(old_name, new_name, new_info):
                self.show_message_box("The entry is already in the book.", "error", "Caution!")
                return
            else:
    ...
    def find_entry(self):
        ...
        def confirm():
            name, _ = self.get_current_name_and_info()
    ...
    def get_name_and_info_by_index(self):
        return self.database.get_contact_by_index(self.current_index, self.sort)

    def get_current_name_and_info(self):
        return self.lineEdit_name.text(), self.textEdit_info.toPlainText()
