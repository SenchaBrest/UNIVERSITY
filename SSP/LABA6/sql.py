...
class ClientBookDatabase:
    ...
    def add_contact(self, name, info):
        try:
            self.cursor.execute("INSERT INTO contacts (name, info) VALUES (?, ?)", (name, info))
            self.connection.commit()
            return True
        except sqlite3.Error:
            return False

    def edit_contact(self, old_name, new_name, new_info):
        try:
            self.cursor.execute("UPDATE contacts SET name=?, info=? WHERE name=?", (new_name, new_info, old_name))
            self.connection.commit()
            return True
        except sqlite3.Error:
            return False
